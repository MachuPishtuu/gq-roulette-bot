...
# In-memory last rolled units to prevent immediate repeats
...
# Individual reroll commands (lead, side1, side2) follow the same pattern
...
async def phases(ctx):
    units = load_units()
    if not units:
        await ctx.send("❌ No phases found.")
        return
    phases_list = ", ".join(sorted(units.keys()))
    await ctx.send(f"🗂️ **Available Phases:** {phases_list}")
...
# ================= RUN BOT =================
bot.run(TOKEN)    with open("units.csv", newline='', encoding="utf-8") as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            phase = row["Phase"].strip()
            if not phase:
